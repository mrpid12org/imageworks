...
def _pairwise(entries: Iterable[JudgeVisionEntry]) -> List[tuple[JudgeVisionEntry, JudgeVisionEntry]]:
    queue = list(entries)
...
def _rank_by_total(entries: Sequence[JudgeVisionEntry]) -> List[dict]:
    ...
    return [
        {"image": entry.image, "score": float(entry.total)} for entry in ordered
    ]
